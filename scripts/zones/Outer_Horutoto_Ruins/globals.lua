...
OUTER_HORUTOTO_RUINS = {
    ...
    gizmoOnTrigger = function(player, npc)
        ...
        if player:getCurrentMission(WINDURST) == tpz.mission.id.windurst.THE_HEART_OF_THE_MATTER then
            ...
            if missionStatus == 2 then
                if player:getMissionStatus(WINDURST, gizmoNum) == 1 then
            ...
            elseif missionStatus == 4 then
                if player:getMissionStatus(WINDURST, gizmoNum) == 2 then
    ...
    gizmoOnEventFinish = function(player, csid)
        ...
        if npc then
            ...
            local msgBase = ID.text.ORB_ALREADY_PLACED
            local orbVal = player:getMissionStatus(WINDURST, gizmoNum)
            ...
            if csid == (57 + gizmoNum) then
                if orbVal == 1 then
                    ...
                    player:setMissionStatus(WINDURST, gizmoNum, 2)
                    ...
                    if
                        player:getMissionStatus(WINDURST, 1) == 2 and
                        player:getMissionStatus(WINDURST, 2) == 2 and
                        player:getMissionStatus(WINDURST, 3) == 2 and
                        player:getMissionStatus(WINDURST, 4) == 2 and
                        player:getMissionStatus(WINDURST, 5) == 2 and
                        player:getMissionStatus(WINDURST, 6) == 2
                    then
                        player:messageSpecial(msgBase + 5) -- "You have set all of the Dark Mana Orbs in place."
                        player:setMissionStatus(WINDURST, 0, 3)
                    end
                end
            ...
            elseif csid == (45 + gizmoNum) then
                if orbVal == 2 then
                    ...
                    player:setMissionStatus(WINDURST, gizmoNum, 3)
                    player:addKeyItem(ki)
                    player:messageSpecial(ID.text.KEYITEM_OBTAINED, ki)

                    if
                        player:getMissionStatus(WINDURST, 1) == 3 and
                        player:getMissionStatus(WINDURST, 2) == 3 and
                        player:getMissionStatus(WINDURST, 3) == 3 and
                        player:getMissionStatus(WINDURST, 4) == 3 and
                        player:getMissionStatus(WINDURST, 5) == 3 and
                        player:getMissionStatus(WINDURST, 6) == 3
                    then
                        player:messageSpecial(msgBase + 4)
                        player:setMissionStatus(WINDURST, 0, 5)
                    end
                end
            end
        end
